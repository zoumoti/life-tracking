...
function getDeadlineStatus(objective: Tables<"objectives">, c: ThemeColors): {
  label: string;
  color: string;
} {
  ...
  if (now > deadline) {
    return actualProgress >= 1
      ? { label: "Atteint !", color: c.success }
      : { label: "Expire", color: c.danger };
  }

  const diff = actualProgress - expectedProgress;
  if (diff >= 0) {
    return { label: "En avance", color: c.success };
  } else if (diff > -0.15) {
    return { label: "En rythme", color: c.textSecondary };
  } else {
...
export function ObjectiveRow({ objective, visionColor, onPress }: Props) {
  ...
  const progress =
    objective.target_value && objective.target_value > 0
      ...
      : 0;
  const status = getDeadlineStatus(objective, c);

  return (
    <Pressable onPress={onPress} className="py-3 active:opacity-80">
      <View className="flex-row items-center justify-between mb-1">
        <Text className="text-base flex-1 mr-2" style={{ color: c.text }} numberOfLines={1}>
          {objective.title}
        </Text>
        {status.label ? (
      ...
      <ProgressBar progress={progress} color={visionColor ?? c.primary} className="mb-1" />
      ...
      <View className="flex-row items-center justify-between">
        ...
        {objective.deadline && (
          <Text className="text-xs" style={{ color: c.textMuted }}>{formatDeadline(objective.deadline)}</Text>
        )}
      </View>
    </Pressable>
  );
