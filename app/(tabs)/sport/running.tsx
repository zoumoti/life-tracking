  ...
  return (
    <SafeScreen>
      <View className="flex-row items-center justify-between mb-4">
        ...
        <Text className="text-text text-xl font-bold">Course a pied</Text>
      ...
      <FlatList
        ...
        ListHeaderComponent={
          <View>
            <View className="flex-row justify-around mb-4 bg-surface rounded-card py-4">
              <View className="items-center">
                <Text className="text-text font-bold text-lg">
                  {Math.round(stats.thisWeekDistance * 10) / 10} km
                </Text>
                <Text className="text-text-secondary text-xs">Cette semaine</Text>
              </View>
              <View className="items-center">
                <Text className="text-text font-bold text-lg">
                  {stats.avgPace > 0 ? formatPace(stats.avgPace, 1) : "--:--"}
                </Text>
                <Text className="text-text-secondary text-xs">Allure moy.</Text>
              </View>
              <View className="items-center">
                <Text className="text-text font-bold text-lg">{stats.totalRuns}</Text>
                <Text className="text-text-secondary text-xs">Courses</Text>
              </View>
            </View>

            {weeklyData.some((w) => w.value > 0) && (
              <Card className="mb-4">
                <Text className="text-text font-bold text-sm mb-3">Distance / semaine</Text>
                <SimpleBarChart data={weeklyData} unit=" km" />
              </Card>
            )}

            <Text className="text-text-secondary text-sm mb-2">Historique</Text>
          </View>
        }
        renderItem={({ item }) => (
          <Card className="mb-2">
                ...
                {Array.from({ length: 5 }).map((_, i) => (
                  <View
                    ...
                    className={`w-2 h-2 rounded-full mx-0.5 ${
                      i < item.perceived_effort ? "bg-primary" : "bg-surface-light"
                    }`}
                  />
                ))}
              </View>
        ...
        ListEmptyComponent={
          !loading ? (
            <View className="items-center mt-8">
              ...
              <Text className="text-text-secondary mt-4">Aucune course</Text>
              <Button
